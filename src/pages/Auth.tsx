  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === 'login') {
      ...
      if (error) {
        ...
        setLoading(false) // Only reset local loading if it fails
      } else {
        onNavigate('home')
      }
    } else {
        ...
        <div className='bg-white rounded-3xl border border-stone-100 shadow-sm p-8'>
          ...
          <form onSubmit={handleSubmit} className='space-y-4'>
            ...
            <button
              type='submit'
              disabled={loading}
              className='w-full flex items-center justify-center gap-2 py-4 bg-stone-900 hover:bg-amber-600 text-white font-bold rounded-xl transition-all text-sm mt-2 disabled:opacity-50'
            >
              {loading
                ? 'Please wait...'
                : mode === 'login'
