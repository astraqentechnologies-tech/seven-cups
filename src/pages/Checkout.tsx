...
import { CheckCircle, ArrowRight, Package, CreditCard, ChevronLeft } from 'lucide-react';
...
const countries = ['United States', 'United Kingdom', 'Canada', 'Australia', 'Germany', 'France', 'Japan', 'Singapore', 'India', 'Other'];
...
export default function Checkout({ onNavigate }: Props) {
    ...
    return (
      <div className="min-h-screen bg-stone-50 pt-20 flex items-center justify-center">
        <div className="text-center">
          ...
          <h2 className="text-2xl font-bold text-stone-900 font-serif mb-3">Sign In to Checkout</h2>
          <p className="text-stone-500 mb-6">Please sign in to complete your purchase.</p>
          <button onClick={() => onNavigate('auth')} className="px-8 py-4 bg-stone-900 text-white font-bold rounded-full hover:bg-amber-600 transition-all">
            Sign In
          </button>
        </div>
      </div>
  ...
  const handlePlaceOrder = async () => {
    if (displayItems.length === 0 && localItems.length === 0) return;
    setPlacing(true);
    ...
    return (
      <div className="min-h-screen bg-stone-50 pt-20 flex items-center justify-center px-4">
        <div className="max-w-lg w-full text-center">
          ...
          <h1 className="text-4xl font-bold text-stone-900 font-serif mb-3">Order Confirmed!</h1>
          <p className="text-stone-500 mb-3">Thank you for your order. We're preparing it with care.</p>
          ...
          <p className="text-stone-400 text-sm mb-10">A confirmation will be sent to <strong>{form.email}</strong>. Your order will arrive within 5–7 business days.</p>
          <div className="flex gap-4 justify-center">
            <button onClick={() => onNavigate('orders')} className="px-8 py-4 bg-stone-900 hover:bg-amber-600 text-white font-bold rounded-full transition-all">
              Track My Order
            </button>
            <button onClick={() => onNavigate('products')} className="px-8 py-4 border border-stone-200 text-stone-700 font-semibold rounded-full hover:border-amber-400 hover:text-amber-700 transition-all">
              Continue Shopping
            </button>
          </div>
        </div>
    ...
    <div className="min-h-screen bg-stone-50 pt-20">
      <div className="max-w-6xl mx-auto px-6 py-12">
        ...
        <div className="flex items-center justify-center gap-4 mb-12">
          {[{ n: 1, label: 'Shipping' }, { n: 2, label: 'Review & Pay' }].map((s, i) => (
            <div key={s.n} className="flex items-center gap-4">
              <div className="flex items-center gap-2">
                <div className={`w-9 h-9 rounded-full flex items-center justify-center font-bold text-sm transition-all ${step >= s.n ? 'bg-stone-900 text-white' : 'bg-stone-200 text-stone-500'}`}>
                  {step > s.n ? <CheckCircle className="w-5 h-5" /> : s.n}
                </div>
                <span className={`text-sm font-medium ${step >= s.n ? 'text-stone-900' : 'text-stone-400'}`}>{s.label}</span>
              </div>
              {i < 1 && <div className={`w-16 h-0.5 ${step > s.n ? 'bg-stone-900' : 'bg-stone-200'}`} />}
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-10">
          ...
          <div className="lg:col-span-3">
            ...
            {step === 2 && (
              <div className="bg-white rounded-3xl border border-stone-100 p-8 shadow-sm">
                <button onClick={() => setStep(1)} className="flex items-center gap-2 text-stone-500 hover:text-amber-600 text-sm mb-6 transition-colors">
                ...
                <div className="mb-6">
                  ...
                  <div className="space-y-3">
                    {[
                      ...
                      <label key={pm.value} className={`flex items-start gap-3 p-4 rounded-xl border cursor-pointer transition-all ${form.payment === pm.value ? 'border-amber-400 bg-amber-50' : 'border-stone-200 hover:border-amber-300'}`}>
                        <input type="radio" value={pm.value} checked={form.payment === pm.value} onChange={e => setForm(f => ({ ...f, payment: e.target.value }))} className="mt-0.5" />
                        <div>
                ...
                <button
                  ...
                  disabled={placing}
                  className="w-full flex items-center justify-center gap-2 py-4 bg-amber-500 hover:bg-amber-400 text-stone-900 font-bold rounded-xl transition-all text-sm"
                >
            ...
            <div className="bg-white rounded-3xl border border-stone-100 p-6 shadow-sm sticky top-24">
                ...
                {displayItems.map((item, i) => (
                  <div key={i} className="flex gap-3 items-center">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-stone-800 font-semibold text-sm font-serif line-clamp-1">{item.name}</p>
                      <p className="text-stone-400 text-xs">Qty: {item.quantity}</p>
                    </div>
                    <p className="text-stone-800 font-semibold text-sm shrink-0">${(item.price * item.quantity).toFixed(2)}</p>
                  </div>
                ))}
              </div>
              <div className="border-t border-stone-100 pt-4 space-y-2.5">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className={shipping === 0 ? 'text-emerald-600 font-semibold' : 'text-stone-800'}>
