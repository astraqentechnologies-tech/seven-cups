...
export default function Admin () {
  ...
  const [productForm, setProductForm] = useState<Partial<LaravelProduct>>(EMPTY_PRODUCT)
                  ...
                  {orders.slice(0, 5).map(order => {
                    ...
                    return (
                      <div key={order.id} className='flex items-center justify-between py-2 border-b border-stone-50'>
                        <div>
                          <p className='font-mono text-stone-800 text-sm font-semibold'>#LUM-{order.id}</p>
                          <p className='text-stone-400 text-xs'>{new Date(order.created_at).toLocaleDateString()}</p>
                        </div>
                        <div className='text-right'>
                          <p className='font-bold text-stone-800 text-sm'>₹{totalAmt.toFixed(2)}</p>
                          <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wider ${
                            { pending: 'bg-yellow-100 text-yellow-700', delivered: 'bg-emerald-100 text-emerald-700', shipped: 'bg-blue-100 text-blue-700' }[order.status] || 'bg-stone-100 text-stone-600'
                          }`}>
                            {order.status}
                          </span>
                        </div>
        ...
        {activeTab === 'products' && (
          <div>
            <div className='flex justify-between items-center mb-5'>
              ...
              <button
                onClick={() => { setProductForm(EMPTY_PRODUCT); setEditingProduct(null); setProductModal(true) }}
                  ...
                  <tbody>
                    {products.map(p => (
                      <tr key={p.id} className='border-b border-stone-50 hover:bg-stone-50/50 transition-colors'>
                        ...
                        <td className='px-4 py-3'>
                          <p className='font-bold text-stone-800'>₹{Number(p.price || 0).toFixed(2)}</p>
                          {Number(p.compare_price || 0) > 0 && (
                            <p className='text-stone-400 text-xs line-through'>₹{Number(p.compare_price).toFixed(2)}</p>
                          )}
                        </td>
                        <td className='px-4 py-3'>
        ...
        {activeTab === 'categories' && (
          <div>
            <div className='flex justify-between items-center mb-5'>
              ...
              <button
                onClick={() => { setCatForm({ name: '', slug: '', description: '', image_url: '' }); setEditingCat(null); setCatModal(true) }}
                className='flex items-center gap-2 px-5 py-2.5 bg-stone-900 hover:bg-amber-600 text-white font-semibold rounded-full text-sm transition-all'
              >
                ...
                <div key={cat.id} className='bg-white rounded-2xl border border-stone-100 overflow-hidden shadow-sm'>
                  ...
                  <div className='p-5'>
                    <div className='flex items-start justify-between mb-2'>
                      ...
                      <div className='flex gap-2'>
                        <button
                          onClick={() => { setCatForm({ name: cat.name, slug: cat.slug, description: cat.description || '', image_url: cat.image_url || '' }); setEditingCat(cat.id); setCatModal(true) }}
                  ...
                  <tbody>
                    {orders.map(order => (
                      <tr key={order.id} className='border-b border-stone-50 hover:bg-stone-50/50'>
                        <td className='px-4 py-3 font-mono font-semibold text-stone-800 text-xs'>#LUM-{order.id}</td>
                        <td className='px-4 py-3'>
                          <p className='font-medium text-stone-700'>{order.full_name}</p>
                        ...
                        <td className='px-4 py-3 font-bold text-stone-800'>${Number(order.total_amount || 0).toFixed(2)}</td>
                        <td className='px-4 py-3'>
                          <span className={`text-[10px] uppercase font-bold px-2 py-1 rounded-full ${order.status === 'delivered' ? 'bg-emerald-100 text-emerald-700' : 'bg-yellow-100 text-yellow-700'}`}>
                        ...
                        <td className='px-4 py-3'>
                          <span className='text-stone-400 text-xs'>{order.items?.length || 0} item(s)</span>
                        </td>
                      </tr>
                    ))}
                  </tbody>
              ...
              <div className='grid grid-cols-1 sm:grid-cols-2 gap-5'>
                {[
                  ...
                  <div key={field.key} className={field.span === 2 ? 'col-span-2' : ''}>
                    ...
                    {field.textarea ? (
                      <textarea
                        value={String((productForm as Record<string, any>)[field.key] || '')}
                ...
                <div className='col-span-2 flex flex-wrap gap-4'>
                  {[
                    ...
                    <label key={flag.key} className='flex items-center gap-2 cursor-pointer'>
                      <input
                        type='checkbox'
                        checked={Boolean((productForm as Record<string, any>)[flag.key])}
