...
import {
  ShoppingCart, ChevronLeft, Plus, Minus, Star, Leaf,
  Clock, Thermometer, Package, CheckCircle, Shield, Truck
} from 'lucide-react'
import { useNavigate } from 'react-router-dom'
import { useCart } from '../context/CartContext'
import { useAuth } from '../context/AuthContext'
import type { ApiProduct, ApiImage } from './Home'
...
function normalizeImageUrl(img: ApiImage): string {
...
export default function ProductDetail({ slug }: Props) {
  ...
  useEffect(() => {
    setLoading(true)
    setActiveImage(0)
    fetch(`${BASE_URL}/api/products/${slug}`)
      .then(res => { if (!res.ok) throw new Error(`HTTP ${res.status}`); return res.json() })
      .then((data: ApiProduct) => setProduct(data))
      .catch(err => { console.error('Product fetch failed:', err); setProduct(null) })
    ...
    try {
      const res = await fetch(`${BASE_URL}/api/reviews`, {
        ...
        body: JSON.stringify({
          ...
          reviewer_name: (profile as any)?.name || (profile as any)?.full_name || 'Anonymous',
          ...reviewForm
        })
      })
  ...
  const images: string[] = sortedImages.length > 0
    ? sortedImages.map(normalizeImageUrl)
  ...
  const productReviews: ProductReview[] = (product as any).reviews || []
  ...
  const brewingMeta = [
    ...
    product.temperature && { icon: Thermometer, label: 'Temperature', value: `${product.temperature}°C` },
            ...
            {avgRating && (
              <div className="flex items-center gap-2 mb-4">
                <div className="flex gap-0.5">
                  {[1,2,3,4,5].map(n => (
                    <Star key={n} className={`w-4 h-4 ${n <= Math.round(Number(avgRating)) ? 'text-amber-400 fill-amber-400' : 'text-stone-200 fill-stone-200'}`} />
                  ))}
                </div>
            ...
            <div className="flex items-center gap-3 mb-5">
              ...
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-2 py-3.5 rounded-2xl font-bold text-sm transition-all shadow-sm ${
                  added ? 'bg-emerald-500 text-white' : 'bg-stone-900 hover:bg-amber-600 text-white active:scale-95'
                }`}
              >
                {added ? (
        ...
        <div className="mt-12 bg-white rounded-3xl border border-stone-100 shadow-sm overflow-hidden">
            ...
            {TABS.map(tab => (
              <button
                ...
                className={`shrink-0 px-6 py-4 text-sm capitalize transition-colors relative ${
                  activeTab === tab ? 'text-amber-600' : 'text-stone-400 hover:text-stone-600 font-semibold'
                }`}
              >
                {tab}
          ...
          <div className="p-6 sm:p-8">
            ...
            {activeTab === 'ingredients' && (
              <div>
                <p style={arialBlack} className="text-xs text-stone-400 uppercase tracking-wide mb-3">Ingredients</p>
                <p className="text-stone-700 leading-relaxed text-sm sm:text-base">
                  {product.ingredients || 'Pure single-origin tea leaves.'}
                </p>
                {product.flavor_profile && (
                  <div className="mt-6 p-4 bg-amber-50 rounded-2xl border border-amber-100">
                    <p style={arialBlack} className="text-amber-700 text-xs uppercase tracking-wide mb-1">Flavor Profile</p>
            ...
            {activeTab === 'brewing' && (
              <div>
                <p style={arialBlack} className="text-xs text-stone-400 uppercase tracking-wide mb-3">How to Brew</p>
                      ...
                      <div key={r.id} className="p-4 sm:p-5 bg-stone-50 rounded-2xl border border-stone-100">
                        <div className="flex items-start justify-between gap-3 mb-2">
                          ...
                          <div className="flex gap-0.5 shrink-0">
                            {[1,2,3,4,5].map(n => (
                              <Star key={n} className={`w-3.5 h-3.5 ${n <= r.rating ? 'text-amber-400 fill-amber-400' : 'text-stone-200 fill-stone-200'}`} />
                            ))}
                          </div>
                        </div>
                          ...
                          <div className="flex gap-1">
                            {[1,2,3,4,5].map(n => (
                              <button key={n} type="button" onClick={() => setReviewForm(f => ({ ...f, rating: n }))} className="p-1">
                                <Star className={`w-6 h-6 transition-colors ${n <= reviewForm.rating ? 'text-amber-400 fill-amber-400' : 'text-stone-200 fill-stone-200 hover:text-amber-300'}`} />
                              </button>
                            ))}
                          </div>
                        </div>
