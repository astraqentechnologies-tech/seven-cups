...
const CATEGORIES = [...new Map(
  PRODUCTS.flatMap(p =>
    p.category
      ? [{ name: p.category.name, slug: p.category.name.toLowerCase().replace(/\s+/g, '-') }]
      : []
  ).map(c => [c.slug, c])
).values()]
...
export default function Products() {
  ...
  const products = useMemo(() => {
    ...
    if (sortBy === 'price_asc') result.sort((a, b) => a.price - b.price)
      ...
      result = [...result.filter(p => p.is_new_arrival), ...result.filter(p => !p.is_new_arrival)]
    else if (sortBy === 'best_seller')
      result = [...result.filter(p => p.is_best_seller), ...result.filter(p => !p.is_best_seller)]
    else
    ...
    <div className='min-h-screen bg-stone-50 pt-16 md:pt-20'>
      ...
      <div className='relative bg-stone-900 overflow-hidden'>
        ...
        <div className='relative px-5 sm:px-6 py-14 sm:py-20 md:py-24 max-w-7xl mx-auto'>
          ...
          <motion.p
            key={activeCategory ? activeCategory.name + '-desc' : 'desc'}
            initial={{ opacity: 0, y: 12 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.55, delay: 0.12 }}
            className='text-stone-300 text-sm sm:text-base max-w-xl leading-relaxed'
        ...
        <div className='flex flex-col md:flex-row gap-6 md:gap-8'>
          ...
          <div className='flex-1 min-w-0'>
            ...
            >
              ...
              <p className='hidden sm:block text-stone-400 text-xs'>
                Sorted by{' '}
                <span className='text-stone-600 font-medium'>{activeSortLabel}</span>
              </p>
            </motion.div>
