    ...
    return (
      <div className="min-h-screen bg-stone-50 pt-20">
        <div className="max-w-3xl mx-auto px-6 py-12">
          <button onClick={() => setSelected(null)} className="flex items-center gap-2 text-stone-500 hover:text-amber-600 text-sm mb-8 transition-colors">
          ...
          <div className="bg-white rounded-3xl border border-stone-100 shadow-sm p-8">
            <div className="flex items-start justify-between mb-6">
              <div>
                ...
                <p className="text-stone-400 text-sm mt-1">{new Date(selected.created_at).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</p>
              </div>
                ...
                {selected.order_items?.map(item => (
                  <div key={item.id} className="flex gap-4 items-center bg-stone-50 rounded-xl p-4">
                    <div className="w-14 h-14 rounded-lg overflow-hidden bg-stone-100 shrink-0">
                      <img src={item.product_image || 'https://images.pexels.com/photos/1638280/pexels-photo-1638280.jpeg'} alt={item.product_name} className="w-full h-full object-cover" />
                    </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-6">
              ...
              <div className="bg-stone-50 rounded-2xl p-5">
                ...
                <div className="space-y-1.5 text-sm">
                  <div className="flex justify-between"><span className="text-stone-500">Subtotal</span><span className="text-stone-800">${selected.subtotal.toFixed(2)}</span></div>
                  <div className="flex justify-between"><span className="text-stone-500">Shipping</span><span className="text-stone-800">{selected.shipping_cost === 0 ? 'Free' : `$${selected.shipping_cost.toFixed(2)}`}</span></div>
                  <div className="flex justify-between font-bold pt-1 border-t border-stone-100 mt-1"><span>Total</span><span>${selected.total.toFixed(2)}</span></div>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="max-w-4xl mx-auto px-6 py-12">
        ...
        {loading ? (
          ...
          <div className="text-center py-20 bg-white rounded-3xl border border-stone-100">
            ...
            <button onClick={() => onNavigate('products')} className="px-8 py-4 bg-stone-900 text-white font-bold rounded-full hover:bg-amber-600 transition-all">
              Browse Teas
            </button>
          </div>
        ) : (
            ...
            {orders.map(order => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-stone-400 text-xs">{new Date(order.created_at).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' })}</p>
                </div>
